...
function crearRespuestaSaludo(valor) {
  const entrada = normalizarEntrada(valor);
  const saludos = new Map([
    ["hola", "Hola."],
  ...
  const saludo = saludos.get(entrada);

  if (!saludo) {
    return null;
  }

  return `${saludo} ¿Quieres reservar, consultar, modificar o cancelar una reserva?`;
}


function esIntencionReserva(valor) {
  const entrada = normalizarEntrada(valor);

  return /\b(?:reserv\w*|recerv\w*|reserb\w*|riserv\w*|mesa)\b/.test(entrada);
}
...
function extraerPersonas(valor) {
  ...
  const patrones = [
    ...
    new RegExp(`\\b(?:para|pa)\\s+${numero}(?:\\s+personas?)?\\b`),
    new RegExp(`\\b(?:somos|seremos)\\s+${numero}\\b`),
    new RegExp(`^${numero}$`)
  ];
...
function analizarPersonas(valor) {
  const entrada = normalizarEntrada(valor);
  const numero =
    "(?:\\d{1,2}|una|uno|dos|tres|cuatro|cinco|seis|siete|ocho|nueve|diez|once|doce)";
  const patrones = [
    new RegExp(`\\b${numero}\\s+(?:personas?|comensales?)\\b`, "g"),
    new RegExp(`\\b(?:somos|seremos)\\s+${numero}\\b`, "g"),
    new RegExp(`\\b(?:para|pa)\\s+${numero}(?!\\s+(?:de|por)\\s+la\\s+(?:manana|tarde|noche))\\b`, "g")
  ];
  const candidatos = [];

  for (const patron of patrones) {
    for (const coincidencia of entrada.matchAll(patron)) {
      const personas = extraerPersonas(coincidencia[0]);

      if (Number.isInteger(personas) && personas > 0) {
        candidatos.push(personas);
      }
    }
  }

  if (/\bo\b/.test(entrada) && /\b(?:personas?|comensales?)\b/.test(entrada)) {
    for (const fragmento of entrada.split(/\bo\b/)) {
      const personas = extraerPersonas(`${fragmento} personas`);
  ...
  const valores = [...new Set(candidatos)];

  if (valores.length > 1) {
    return { estado: "ambiguo", valor: null };
  }

  if (valores.length === 1) {
    return { estado: "seguro", valor: valores[0] };
  }
...
function extraerHora(valor, permitirRespuestaBreve = false) {
  ...
  const numeroHora =
    "(\\d{1,2}|una|uno|dos|tres|cuatro|cinco|seis|siete|ocho|nueve|diez|once|doce|trece|catorce|quince|dieciseis|diecisiete|dieciocho|diecinueve|veinte|veintiuna|veintiuno|veintidos|veintitres)";
  const horaNatural = texto.match(new RegExp(
    "\\b(?:a\\s+(?:eso\\s+de\\s+)?|sobre\\s+|hacia\\s+|para\\s+)" +
    `las?\\s+${numeroHora}` +
    "(?!\\s+personas?\\b)" +
    "(?:\\s+y\\s+(media|cuarto))?" +
    "(?:\\s+horas?)?" +
  ...
  const horaNaturalConPeriodo = normalizarEntrada(original).match(new RegExp(
    `\\b${numeroHora}` +
    "(?:\\s+y\\s+(media|cuarto))?" +
    "\\s+(?:(?:de|por)\\s+la\\s+)(manana|tarde|noche|madrugada)\\b"
  ));
  const horaBreve = permitirRespuestaBreve
    ? normalizarEntrada(original).match(
      new RegExp(`^${numeroHora}(?:\\s+horas?)?$`)
    )
    : null;
  const coincidencia = horaNatural || horaNaturalConPeriodo || horaBreve;
  ...
  const tienePeriodoNatural = coincidencia === horaNatural ||
    coincidencia === horaNaturalConPeriodo;
  const minutos = tienePeriodoNatural
    ? coincidencia[2] === "media"
      ...
      : coincidencia[2] === "cuarto"
        ...
        : 0
    : 0;
  const periodo = tienePeriodoNatural
    ? coincidencia[3] || ""
    : "";
...
function analizarHora(valor) {
  const candidatos = [extraerHora(valor)].filter(Boolean);
  const fragmentosAlternativos = normalizarEntrada(valor).split(/\bo\b/);

  if (fragmentosAlternativos.length > 1) {
    for (const fragmento of fragmentosAlternativos) {
      const hora = extraerHora(fragmento, true);

      if (hora) {
        candidatos.push(hora);
      }
    }
  }
...
function detectarCampoCorreccion(valor) {
  const entrada = normalizarEntrada(valor);

  if (!/\b(?:cambi\w*|modific\w*|correg\w*)\b/.test(entrada)) {
    return null;
  }

  if (/\b(?:hora|horario)\b/.test(entrada)) {
    return "hora";
  }

  if (/\b(?:fecha|dia)\b/.test(entrada)) {
    return "fecha";
  }

  if (/\b(?:persona|personas|comensal|comensales)\b/.test(entrada)) {
    return "personas";
  }

  if (/\b(?:zona|terraza|interior|sala)\b/.test(entrada)) {
    return "zona";
  }
...
function interpretarRespuestaBinaria(valor) {
  const respuesta = normalizarEntrada(valor).replace(/\bzi\b/g, "si");
...
function interpretarValidacionDatos(valor) {
  const respuesta = normalizarEntrada(valor).replace(/\bzi\b/g, "si");
  ...
  if (
    detectarCampoCorreccion(respuesta) ||
    /\b(?:pero|salvo|excepto|incorrecto|incorrectos|mal)\b/.test(respuesta) ||
    /^(?:no|n)\b/.test(respuesta)
  ) {
    return "no";
  }

  if (
    /^(?:si|s)(?:\s+(?:es|esta|todo|son|los datos son))*\s*(?:correcto|correctos|bien)?$/.test(respuesta) ||
    /^(?:si\s+){1,}si$/.test(respuesta) ||
    /^(?:los\s+datos\s+)?son\s+correctos?$/.test(respuesta) ||
    /^(?:si\s+)?confirmo(?:\s+los\s+datos)?$/.test(respuesta) ||
    /^(?:todo\s+)?(?:correcto|correctos|esta\s+bien)$/.test(respuesta) ||
    /^(?:de\s+acuerdo|vale|adelante)$/.test(respuesta)
  ) {
    return "si";
  }
