...
function extraerHora(valor, permitirRespuestaBreve = false) {
  ...
  const horaNaturalSinPreposicion = permitirRespuestaBreve
    ? normalizarEntrada(original).match(new RegExp(
      `^(?:las?\\s+)?${numeroHora}` +
      "(?:\\s+y\\s+(media|cuarto))?" +
      "\\s+(?:de\\s+la\\s+)?(manana|tarde|noche|madrugada)$"
    ))
    : null;
  const horaBreve = permitirRespuestaBreve
    ? normalizarEntrada(original).match(new RegExp(`^${numeroHora}$`))
    : null;
  const coincidencia = horaNatural || horaNaturalSinPreposicion || horaBreve;
  ...
  const tienePeriodoNatural = coincidencia === horaNatural ||
    coincidencia === horaNaturalSinPreposicion;
