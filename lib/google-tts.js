...
const MINUTOS_EN_PALABRAS = Object.freeze({
  "00": "",
  "15": " y quince minutos",
...
const DIGITOS_EN_PALABRAS = Object.freeze([
...
function vozGoogleValida(voz) {
  const nombre = String(voz || "");

  return VOCES_GOOGLE_PERMITIDAS.includes(nombre) ||
    nombre === "es-ES-Studio-C";
}


function verbalizarHoraConfirmada(texto) {
  const confirmacion = String(texto || "").replace(
    /He entendido las (\d{2}):(00|15|30|45) horas\./g,
    (coincidencia, hora, minutos) => {
      const numeroHora = Number(hora);
      const horaHablada = HORAS_EN_PALABRAS[numeroHora];

      if (!horaHablada) {
        return coincidencia;
      }

      const articulo = numeroHora === 1 ? "la" : "las";
      const sufijoHora = numeroHora === 1 ? "" : " horas";
      return `He entendido ${articulo} ${horaHablada}${sufijoHora}` +
        `${MINUTOS_EN_PALABRAS[minutos]}.`;
    }
  );

  return confirmacion.replace(
    ...
    (coincidencia, hora, minutos) => {
      ...
      const sufijoHora = numeroHora === 1 ? "" : " horas";
      return `${horaHablada}${sufijoHora}${MINUTOS_EN_PALABRAS[minutos]}`;
    }
  );
}


function verbalizarTelefonos(texto) {
  return String(texto || "").replace(
    /(?:\+34[\s.-]*([67]\d{8})|\b([67]\d{8})\b)/g,
    (coincidencia, telefonoConPrefijo, telefonoSinPrefijo) => {
      const telefono = telefonoConPrefijo || telefonoSinPrefijo;

      return telefono
        .split("")
        .map((digito) => DIGITOS_EN_PALABRAS[Number(digito)])
        .join(", ");
    }
  );
}
...
function verbalizarCorreos(texto) {
  return String(texto || "").replace(
    /[\w.%+-]+@[\w.-]+\.[a-z]{2,}/giu,
    (correo) => correo
      .replace("@", " arroba ")
...
function prepararTextoParaVoz(texto) {
  return verbalizarTelefonos(
    verbalizarCorreos(verbalizarHoraConfirmada(texto))
  )
    .replace(
      /\[[^\]]*\]\(https?:\/\/[^)]+\)/giu,
      " El enlace aparece en pantalla. "
    )
    .replace(/https?:\/\/\S+/giu, " El enlace aparece en pantalla. ")
    .replace(
...
async function sintetizarVozGoogle({
  ...
  const contenido = prepararTextoParaVoz(texto);
